...
func _get_label_by_action(action: ActionNode) -> Dictionary:
	for element: Dictionary in label_dict.values():
		if element[&"action_name"] == action.name:
			return element
	return {}
...
func _on_action_map_changed(action_map: Dictionary[StringName, StringName]) -> void:
	...
	for request: StringName in action_map.keys():
		...
		var action: ActionNode = action_container.action_dict[action_name] if action_container.action_dict.has(action_name) else null
		var label: Label = _create_label((request + ": " + action_name), ACTIVE_COLOR_TXT, ACTIVE_COLOR_BG) if action else \
			_create_label((request + ": " + action_name), INACTIVE_COLOR_TXT, INACTIVE_COLOR_BG)
		action_ui_container.add_child(label)
		label_dict[request] = {&"label":label, &"timestamp":Time.get_ticks_msec(), &"timer":null, &"action_name":action_name, &"request":request}
...
func _on_action_container_child_entered(node:Node) -> void:
	...
	for request: StringName in action_player.action_map.keys():
		if action_player.action_map[request] == action.name:
			label_dict[request][&"action_name"] = action.name
